...
var dataModels ={
	...
	getRevista: function(idEjemplar,callback){
		 if(connection)
		{
			var sql ='SELECT*FROM c_ejemplar INNER JOIN c_revista ON c_ejemplar.id_ejemplar = c_revista.id_revista INNER JOIN c_autores ON c_ejemplar.id_autor = c_autores.id_autor where c_ejemplar.id_ejemplar ='+connection.escape(idEjemplar)+'';
			connection.query(sql, function(error, row) 
			{
				if(error)
				{
